...
def trackOpt(decimator, allCorners, allIds, cap, cameraMatrix, distCoeffs, dictionary, board):
    ...
    if data == 1:
        ...
        decideControl(decimator, allCorners, allIds, cap, cameraMatrix, distCoeffs, dictionary, board, Ji)
    elif data == 2:
...
def decideControl(decimator, allCorners, allIds, cap, cameraMatrix, distCoeffs, dictionary, board, Ji):
    print("***************************************************")
    print("\nPlease choose from the following:")
    print("\n[1] Control Robot")
    print("[2] Exit")
    ...
    if data == 1:
        print("\nControlling Robot")
        ControlRobot(decimator, allCorners, allIds, cap, cameraMatrix, distCoeffs, dictionary, board, Ji)
    else:
        print("\nThanks for visiting!")
    return ()
...
### minJerk Control Robot Function
def ControlRobot(decimator, allCorners, allIds, cap, cameraMatrix, distCoeffs, dictionary, board, Ji):
    #use state variable to step through different parts of the control
    state = 1
    ...
    x_f = 400 #test position for code
    y_f = 100 #test position for code
    click = False
    ...
    while(cap.isOpened()):
        ...
        # Detect Marker
        res = cv2.aruco.detectMarkers(frame,dictionary)
        cv2.circle(frame,(int(y_f),int(x_f)),5, (0,0,255),-1)
...
##                x_i, y_i = getXY(res[0])
##                cv2.circle(frame,(y_f,x_f),5, (0,0,255),-1)
...
##                state = 1
            ...
            if state == 1: #actual move robot and update Jacobian
                ##### Position stuff #####
                cv2.aruco.drawDetectedMarkers(frame,res[0],res[1])
...
### min jerk traj, creates minimum jerk trajectory in 1D
#Inputs:
#   start = starting value in x or y
#   end = ending value in x or y
#   t = # of steps to get from start to end
#Outputs:
#   trajectory = desired trajectory
##def mjtg(start, end, t):
##    A = np.matrix([[t**3, t**4, t**5],[3*t**2, 4*t**3, 5*t**4],[6*t, 12*t**2, 20*t**3]])
##    b = np.matrix([[end-start],[0],[0]])
...
##    trajectory = []
##    for i in range(1,t):
##        inter = start + a1*(i**3) + a2*(i**4) + a3*(i**5)
##        trajectory.append(inter)
##
##    return trajectory
