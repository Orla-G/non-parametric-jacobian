...
def ControlRobot(cap, dictionary, Ji):
    ...
    while(cap.isOpened()):
        ...
        cv2.circle(frame,(x_f,y_f),5, (0,0,255),-1)
    ...
    try: 
        ...
        with open("control_path_data2.json", "w") as outfile: 
                json.dump(data,outfile)
...
def clicky(click, i,j, flags, param):
    ...
    if click == cv2.EVENT_LBUTTONDOWN:
        ...
        y_f = j
        clikd=True
    elif click==cv2.EVENT_MOUSEMOVE and clikd:
        x_f = i
        y_f = j
    elif click==cv2.EVEN_LBUTTONUP:
        clikd=False
